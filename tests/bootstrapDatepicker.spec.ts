...
test('Booking.com Date Picker Test - Check-in and Check-out', async ({ page }) => {
  ...
  // Handle the intermittent sign-in modal overlay
  const closeButton = page.getByRole('button', { name: 'Dismiss sign-in info.' });
  try {
  ...
  // Click on the date picker field to open calendar using user-facing text
